...
class SideScreenGui extends Gui{

  SegmentGroup guiSegments;
  SegmentGroup refGroup;

  final int xSize = 400;
  final int ySize = 200;

  PGraphics sideCanvas;
  public SideScreenGui(){
    super();
    sideCanvas = createGraphics(xSize, ySize, P2D);
    setupGroups();
  }

  public void setupGroups(){
    guiSegments = new SegmentGroup(-1);
    guiSegments.init();
    int pad = 20;
    guiSegments.startSegment(new PVector(xSize - pad, pad));
    guiSegments.endSegment(new PVector(100, pad));

    guiSegments.startSegment(new PVector(xSize - pad, pad*2));
    guiSegments.endSegment(new PVector(100, pad*2));

    guiSegments.startSegment(new PVector(xSize - pad, pad*3));
    guiSegments.endSegment(new PVector(100, pad*3));

    refGroup = new SegmentGroup(-2);
    refGroup.init();

    refGroup.startSegment(new PVector(pad, pad));
    refGroup.endSegment(new PVector(100, pad));

    refGroup.startSegment(new PVector(100, pad));
    refGroup.endSegment(new PVector(100, 100));

    refGroup.startSegment(new PVector(100, 100));
    refGroup.endSegment(new PVector(pad, 100));

    refGroup.startSegment(new PVector(pad, 100));
    refGroup.endSegment(new PVector(pad, pad));

    refGroup.placeCenter(new PVector(pad+50,pad+50));
  }
  ...
  public void simpleText(Segment _s, int _size){
    ...
    int l = txt.length();
    PVector pos = new PVector(0,0);
    sideCanvas.beginDraw();
    sideCanvas.pushStyle();
    sideCanvas.fill(TEXT_COLOR);
    sideCanvas.noStroke();
    sideCanvas.textFont(font);
    sideCanvas.textSize(_size);
    char[] carr = txt.toCharArray();
    for(int i = 0; i < l; i++){
      pos = _s.getRegPos(-((float)i/(l+1) + 1.0/(l+1))+1);
      sideCanvas.pushMatrix();
      sideCanvas.translate(pos.x, pos.y);
      sideCanvas.rotate(_s.getAngle(false));
      sideCanvas.translate(0,5);
      sideCanvas.text(carr[i], 0, 0);
      sideCanvas.popMatrix();
    }
    sideCanvas.popStyle();
    sideCanvas.endDraw();
  }
