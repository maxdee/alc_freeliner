...
void setup() {
  ...
  println ("Adding " + 100 * slack + "% slack to median distance...");
  println ("Median distance is now "+ medianDistance);
  ...
  for (int i = 1; i < leds.size()-2; i++) {
    ...
    if (startPoint.x > 0 && endPoint.x > 0) {
      ...
      if (dist(startPoint.x, startPoint.y, endPoint.x, endPoint.y) > 2* medianDistance) {
        lineCounter ++;
        stroke(255, 0, 0);
...
public void saveGroups(String _fn) {
  ...
  for (int i = 0; i < startPoints.size(); i++) {
    ...
    // for leds and such
    xseg.setString("txt", "/led " + int(startPoints.get(i).z) + " " + int(endPoints.get(i).z));
  }
